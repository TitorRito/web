...
export default function Dapp() {
  ...
  const handleSendEth = async () => {
    ...
    try {
      ...
      if (!ethers.isAddress(sendEthAddress)) {
        setError('Invalid recipient address');
        setIsSending(false);
        return;
      }

      if (!sendEthAmount || isNaN(Number(sendEthAmount)) || Number(sendEthAmount) <= 0) {
      ...
      const tx = await user.signer.sendTransaction({
        to: sendEthAddress,
        value: ethers.parseEther(sendEthAmount)
      });
  ...
  const handleSwitchNetwork = async (networkConfig: NetworkConfig) => {
    ...
    try {
      ...
      // Network switch will trigger an event handled by MetaMask
      // We'll refresh the user data after a short delay to ensure the switch completed
      setTimeout(async () => {
        try {
          const userData = await getUserProvider();
          setUser(userData);
          
          // If there was a contract, it needs to be recreated with the new network
          if (contract) {
            handleResetContract();
          }
        } catch (error) {
      ...
      if (switchError.code === 4902) {
        try {
          ...
          // Retry getting user data after adding the network
          setTimeout(async () => {
            try {
              const userData = await getUserProvider();
              setUser(userData);
    ...
    <div className='min-h-screen bg-gray-900 text-gray-200 py-10 px-4'>
      <div className='max-w-7xl mx-auto'>
        ...
        <div className="mb-10 text-center opacity-0 translate-y-[-20px] animate-[fadeSlideDown_0.6s_forwards]">
          <h1 className="text-3xl font-bold text-blue-400 mb-2">Blockchain Explorer</h1>
          <p className="text-gray-400 max-w-2xl mx-auto">
            Your gateway to interact with smart contracts and the decentralized web.
          </p>
        </div>
                  ...
                  {!contract && (
                    <div className="text-center mt-4 border-t border-gray-700 pt-4">
                      <p className="text-sm text-blue-400 flex items-center justify-center">
                        ...
                        Create or select a contract to interact with it
                      </p>
                    </div>
                  )}
                </div>
            ...
            {contract ? (
              <div className="bg-gray-800/30 rounded-xl border border-gray-700/50 overflow-hidden shadow-xl">
                <div className="bg-gradient-to-r from-green-900/30 to-blue-900/30 px-4 py-3 border-b border-gray-700 flex items-center">
                  ...
                  <h3 className="font-medium text-green-300">Contract Interface Active</h3>
                </div>
              ...
              <div>
                {user ? (
                  ...
                  <div className="bg-gray-800/30 rounded-xl border border-gray-700/50 p-8 text-center">
                    ...
                    <h3 className="text-xl font-semibold mb-2 text-gray-200">Start Your Blockchain Journey</h3>
                    <p className="text-gray-400 mb-6 max-w-md mx-auto">
                      Connect your wallet to interact with smart contracts, send transactions, and access the decentralized web.
                    </p>
                    <button
        ...
        <div className="mt-12 pt-6 border-t border-gray-800 text-center text-gray-500 text-sm opacity-0 animate-[fadeIn_0.8s_0.6s_forwards]">
          ...
          <p>Your decentralized gateway to the blockchain ecosystem</p>
        </div>
      </div>
