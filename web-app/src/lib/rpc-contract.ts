...
export async function executeContract({
    ...
    try {
        ...
        // Get function info
        const functionFragment = contract.instance.interface.getFunction(functionName);
        if (!functionFragment) {
        ...
        const isWrite = ['nonpayable', 'payable'].includes(functionFragment.stateMutability);
        ...
        if (isRead) {
            ...
            throw new Error(`Unknown state mutability: ${functionFragment.stateMutability}`);
        }
    } catch (e) {
