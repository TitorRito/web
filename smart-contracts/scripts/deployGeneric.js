...
async function main() {
    console.log(
        `\n${COLORS.MAGENTA}HARDHAT ${COLORS.RESET} to deploy ${COLORS.YELLOW}${contractsToDeploy.length
        }${COLORS.RESET} contracts: ${COLORS.YELLOW}${contractsToDeploy.join(
            ", "
        )}${COLORS.RESET}\n`
    );
    ...
    for (const contractName of contractsToDeploy) {
        try {
            ...
            console.log(
                `${COLORS.YELLOW}[${contractObj.timestamp}]${COLORS.RESET} Successfully deployed ${COLORS.GREEN}${contractName}${COLORS.RESET} at ${COLORS.GREEN}${contract.target}${COLORS.RESET} on chain ID ${COLORS.GREEN}${providerNetwork.chainId}${COLORS.RESET}`
            );
        } catch (error) {
            console.error(
